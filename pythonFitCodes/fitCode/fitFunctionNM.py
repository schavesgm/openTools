...
def chiSquare(f, args, xData, yData, yError = 0 ):
    '''
        @arg f (function) : function that we would like to fit with the data. Must return a double
                            and be able to accept numpy arrays as input
        @arg args (array) : values that we would like to fit
        @arg xData(array) : xData extracted from the experiment that we would like to fit
        @arg yData(array) : yData extracted from the experiment that we would like to fit
        @arg yError(array): error in the measured function used to calculate the fit function
        return            : double with the chiSquare value at the current status of the coeffs
    '''
    if yError.all() == 0:
...
def fitOnceNM( f, initGuess, xData, yData, yError = 0,
    ...
    '''
        @arg f (function) : function that we would like to optimize. This function must return a
                            double and be able to get numpy arrays as input

        @initGuess (array): array with the initial guess of the points that minimize the function
        @maxIter (int)    : integer with the maximum number of iterations that we accept
        return: tuple     : tuple( Points that minimize the function, minimum value achieved)
    '''
    ...
    # INIT THE ALGORITHM
    N = initGuess.shape[0];               ## Dimensions that we are working in
    evalFunc = chiSquare(f,initGuess,xData,yData,yError)   ## Value of function at the initial guess
    noImprov = 0
    imValues = [[initGuess, evalFunc]]        ## Initial values and its images on the function

    for i in range( N ):                ## Generates the initial simplex appending over initGuess
    ...
    while 1:
        ...
        imValues.sort( key = lambda x: x[1] )   ## Sort using the evaluated function as axis
        ...
        if maxIter and iterStep >= maxIter:
            ...
            if getChiSquare == 0:                ## Control variable to print out the chiSquare
                return bestData
            else:
        ...
        for tup in imValues[:-1]:               ## Get all the tuples in the 'matrix'
            for i, c in enumerate(tup[0]):      ## Disect both tuples, we just need the first one
        ...
        if imValues[0][1] <= refEval < imValues[-2][1]:    ## If the lowest value is lower than
                                                           ## the refEval and refEval is lower
                                                           ## than the second largest value

            del imValues[-1]                               ## Remove the last point
            imValues.append([xRef, refEval])               ## Append the reflection as highest point
            continue

        # EXPANSION PART OF THE ALGORITHM
        if refEval < imValues[0][1]:                       ## If the value of the reflection is
                                                           ## smaller than the lowest value

            xExp = centroidPoint + gamma * ( xRef - imValues[-1][0] )
            expValue = chiSquare(f,xExp,xData,yData,yError)
            if expValue < refEval:                         ## If the value of the expansion is
                                                           ## less than the reflection value
                del imValues[-1]
                imValues.append([xExp, expValue])          ## We append the expansion to the last
                                                           ## position
                continue
            else:
                del imValues[-1]
                imValues.append([xRef, refEval])           ## If not, we append the reflection
                continue
        ...
        if contEval < imValues[-1][1]:                     ## If the contraction image is smaller
        ...
        # REDUCTION PART
        nimValues = []                                     ## We contract everything using the
...
def fitNtimes( f, nReps, initGuess, xData, yData, yError = 0, seed = 1231241 ):
    '''
        Routine that calls "fitOnceNM" nReps times in order to get some statistics. The value of
        initGuess is multiplied by some random value in order to randomize the nCalculations and
        explore the space better.
    '''
