...
void obtPCAC( const struct myMat yDataAP, const struct myMat yDataPP,
              const unsigned numBootstrap, const unsigned timePoints,
              const unsigned initSeed, std::string saveName, int rank, int size ) {
    ...
    for ( unsigned nB = 0; nB < numBootstrap; nB++ ) {
        ...
        sampleFile = pcacMass( yDataAP, yDataPP, timePoints, buffSeed );
